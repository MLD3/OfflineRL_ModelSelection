...
def run_FQE(X, A, X_next, R, n_epoch=10):
    ...
    model = keras.models.load_model('{}/hidden_net'.format(load_dir))
    ...
    for _ in range(n_epoch):
        y = R + gamma * hidden_net.predict(X_next)[range(len(X)), next_actions]
        y = np.clip(y, -1, 1)
        tf.random.set_seed(0)
        hidden_net, Q_net = init_networks()
        Q_net.fit([X, A], y, **fit_args)
    
    Q_net.save('{}/Q_net'.format(save_dir))
    hidden_net.save('{}/hidden_net'.format(save_dir))
    return (hidden_net, Q_net)

def run_FQI(X, A, X_next, R, n_epoch=10):
    def clone_keras_model(model):
        model_copy = keras.models.clone_model(model)
        model_copy.set_weights(model.get_weights())
        return model_copy
