...
def convert_to_policy_table(Q):
    ...
    Handles the last two states (absorbing states)
    """
...
def OPE_WIS_keras(data, model_k, γ, output_dir, epsilon=0.01, split='va'):
    ...
    hidden_net = keras.models.load_model('{}/NFQ-clipped-keras.models/iter={}.hidden_net'.format(output_dir, model_k), compile=False)
...
def get_FQE_value_keras(k, X, output_dir, split):
    try:
        model_FQE = keras.models.load_model('{}/NFQ-clipped-keras.{}FQE_models/model={}.hidden_net'.format(output_dir, split, k), custom_objects={'select_output': select_output}, compile=False)
        return model_FQE.predict(X).max(axis=1)
    except:
        return []
    
def OPE_WDR_FQE_keras(data, model_k, γ, output_dir, epsilon=0.01, split='va'):
    ...
    try:
        model_FQE = keras.models.load_model('{}/NFQ-clipped-keras.{}FQE_models/model={}.hidden_net'.format(output_dir, split, model_k), custom_objects={'select_output': select_output}, compile=False)
    except:
        return np.nan
...
def OPE_AM_keras(policy_value_net_k, X_init, γ, output_dir, rollout=MAX_ROLLOUT, split='va'):
    ...
    policy_value_net = keras.models.load_model('{}/NFQ-clipped-keras.models/iter={}.hidden_net'.format(output_dir, policy_value_net_k), compile=False)
